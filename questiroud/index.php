...
/* ====================== EMAIL CONFIGURATION ============================== */
define("EMAIL_SUBJECT", "Vyhodnocení dotazníku");  // Subject of the email that is send
define("EMAIL_FROM_NAME", "KRS-LK");  // From who (name) it comes
define("EMAIL_FROM_E_MAIL", "krs-lk@seznam.cz");  // From who (e-mail) it comes
define("EMAIL_REPLY_E_MAIL", "krs-lk@seznam.cz");  // To what email reply
/* ========================================================================= */
...
define("TOTAL_SCORE_PREFIX", "Celkové skore: ");  // Total score prefix
define("TOTAL_SCORE_SUFFIX", " procent."); // Total score suffix
define("SUCCESS_MESSAGE", "Váš dotazník byl vyhodnocen. Prosím zkontrolujte svůj email!"); // Message that is shown if everything goes well
/* ========================================================================= */
...
function render_short_email($total_score, $name_of_user, $address_of_user) {
    /**Short email version that just define total score.
    @param total_score: total score achieved for questionnaire in percent
    @param name_of_user: name of the user (filled when post).
    @param address_of_user: e-mail address of user.
    @return: HTML code of response
    */
    $content = render_all_questions_wrapper("", $total_score, 1.0);
    $page_body = render_content_wrapper($content, $name_of_user, $address_of_user);
    return render_layout_wrapper($page_body);
}
// ============================================================================
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!DEBUG_MODE) { // Send email and store info to file
        ...
        // Send email to user
        send_mail_in_utf_8($address, $short_email_body_text);
